...
class BankingTransactionUsageTest {
    ...
    void testSerializeAndDeserializeTransaction() throws Exception {
        ...
        byte[] cobolData = pojoBook.serialize(original, StandardCharsets.ISO_8859_1);
        ...
        BankingTransaction deserialized = pojoBook.deserialize(cobolData, BankingTransaction.class, StandardCharsets.ISO_8859_1);
    ...
    void testTransactionTypes() throws Exception {
        ...
        for (String type : types) {
            ...
            byte[] serialized = pojoBook.serialize(txn, StandardCharsets.ISO_8859_1);
            BankingTransaction deserialized = pojoBook.deserialize(serialized, BankingTransaction.class, StandardCharsets.ISO_8859_1);
    ...
    void testMultipleTransactions() throws Exception {
        ...
        for (int i = 0; i < transactions.length; i++) {
            serializedData[i] = pojoBook.serialize(transactions[i], StandardCharsets.ISO_8859_1);
        }

        // Then: Each should produce different data
        assertFalse(Arrays.equals(serializedData[0], serializedData[1]));
        assertFalse(Arrays.equals(serializedData[1], serializedData[2]));

        // And: Can be deserialized correctly
        for (int i = 0; i < transactions.length; i++) {
            BankingTransaction deserialized = pojoBook.deserialize(
                    serializedData[i], BankingTransaction.class, StandardCharsets.ISO_8859_1);
    ...
    void testSerializationConsistency() throws Exception {
        ...
        byte[] serialized1 = pojoBook.serialize(txn, StandardCharsets.ISO_8859_1);
        byte[] serialized2 = pojoBook.serialize(txn, StandardCharsets.ISO_8859_1);
