...
import org.c4rth.pojobook.samples.generated.SalesOccurs;
 ...
 * Demonstrates OCCURS clause usage - arrays and repeating groups.
...
class SalesOccursUsageTest {
    ...
    void testSerializeAndDeserializeSalesReport() throws Exception {
        ...
        SalesOccurs deserialized = SalesOccurs.deserialize(cobolData, charset);
    ...
    void testMonthlyReports() throws Exception {
        ...
        // When: Serialize each
        byte[] octData = oct.serialize(charset);
        byte[] novData = nov.serialize(charset);
        byte[] decData = dec.serialize(charset);

        // Then: Each should produce different data
        assertNotNull(octData);
        assertNotNull(novData);
        assertNotNull(decData);
        assertFalse(java.util.Arrays.equals(octData, novData));
        assertFalse(java.util.Arrays.equals(novData, decData));

        // When: Deserialize all
        SalesOccurs octResult = SalesOccurs.deserialize(octData, charset);
        SalesOccurs novResult = SalesOccurs.deserialize(novData, charset);
        SalesOccurs decResult = SalesOccurs.deserialize(decData, charset);
    ...
    void testReportPeriodFormats() throws Exception {
        ...
        for (String period : periods) {
            ...
            byte[] serialized = report.serialize(charset);
            SalesOccurs deserialized = SalesOccurs.deserialize(serialized, charset);
    ...
    void testSerializationConsistency() throws Exception {
        ...
        byte[] serialized1 = report.serialize(charset);
        byte[] serialized2 = report.serialize(charset);
